...
def display():
    global ole_Gen_schedule
    global final_schedule
    output = template('display', oldGen=ole_Gen_schedule,final=final_schedule,ole_fitness=ole_Gen_schedule_fitness,final_fitness=final_schedule_fitness)
    return output
...
if __name__ == '__main__':
    ...
    while (ga.fitness(copy.deepcopy(loopGen[0])) <= ole_Gen_schedule_fitness):
        #if count > 100:
            #break
        if ga.fitness(copy.deepcopy(loopGen[1])) >= 150:
            break
        newGen = []
        ...
        for crossoverPopu in crossoverGen2:
            mutationGen.append(ga.mutation(crossoverPopu))
    ...
    final_schedule = copy.deepcopy(loopGen[1])
